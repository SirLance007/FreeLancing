    ...
    return (
        <section id="hero" className="relative min-h-screen flex items-center justify-center overflow-hidden pt-5 sm:pt-24 lg:pt-20 pb-16 sm:pb-20 lg:pb-24 safe-area-top safe-area-bottom">
            ...
            <div className="absolute inset-0 overflow-hidden">
                {/* Large floating orbs */}
                <div 
                    className="absolute top-20 left-10 w-48 h-48 sm:w-72 sm:h-72 bg-gradient-to-br from-blue-400/20 to-purple-400/20 rounded-full blur-3xl animate-float"
                    style={{
                        transform: `translate(${mousePosition.x * 20}px, ${mousePosition.y * 20}px)`
                    }}
                ></div>
                <div 
                    className="absolute bottom-20 right-10 w-64 h-64 sm:w-96 sm:h-96 bg-gradient-to-br from-pink-400/20 to-red-400/20 rounded-full blur-3xl animate-float-delayed"
                    style={{
                        transform: `translate(${-mousePosition.x * 30}px, ${-mousePosition.y * 30}px)`
                    }}
                ></div>
                <div 
                    className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] sm:w-[800px] sm:h-[800px] bg-gradient-to-br from-cyan-400/10 to-blue-400/10 rounded-full blur-3xl animate-float-more-delayed"
                    style={{
                ...
                <div className="absolute top-1/4 right-1/4 w-20 h-20 sm:w-32 sm:h-32 bg-gradient-to-br from-green-400/15 to-teal-400/15 rounded-full blur-2xl animate-float" style={{ animationDelay: '1s' }}></div>
                <div className="absolute bottom-1/4 left-1/4 w-16 h-16 sm:w-24 sm:h-24 bg-gradient-to-br from-purple-400/15 to-pink-400/15 rounded-full blur-2xl animate-float-delayed" style={{ animationDelay: '3s' }}></div>
                <div className="absolute top-3/4 left-1/3 w-24 h-24 sm:w-40 sm:h-40 bg-gradient-to-br from-yellow-400/10 to-orange-400/10 rounded-full blur-2xl animate-float-more-delayed" style={{ animationDelay: '5s' }}></div>
            </div>

            {/* Geometric Shapes */}
            <div className="absolute inset-0">
                {/* Triangles */}
                <div className="absolute top-20 right-4 sm:right-20 w-8 h-8 sm:w-16 sm:h-16 border-2 border-blue-400/20 rotate-45 animate-pulse" style={{ animationDelay: '0.5s' }}></div>
                <div className="absolute bottom-32 left-8 sm:left-32 w-6 h-6 sm:w-12 sm:h-12 border-2 border-purple-400/20 rotate-45 animate-pulse" style={{ animationDelay: '1.5s' }}></div>
                <div className="absolute top-1/2 right-1/3 w-4 h-4 sm:w-8 sm:h-8 border-2 border-green-400/20 rotate-45 animate-pulse" style={{ animationDelay: '2.5s' }}></div>

                {/* Squares */}
                <div className="absolute top-1/3 left-4 sm:left-20 w-6 h-6 sm:w-10 sm:h-10 border-2 border-cyan-400/20 animate-spin" style={{ animationDuration: '20s' }}></div>
                <div className="absolute bottom-1/3 right-4 sm:right-20 w-4 h-4 sm:w-6 sm:h-6 border-2 border-pink-400/20 animate-spin" style={{ animationDuration: '15s', animationDirection: 'reverse' }}></div>
            </div>
                ...
                <div className="text-center max-w-6xl mx-auto">
                    <div className="space-y-8 sm:space-y-12">
                            ...
                            {features.map((feature, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <p className="font-semibold text-gray-900 dark:text-white text-sm sm:text-base leading-tight">{feature.text}</p>
                                    </div>
                                )
                            })}
                        </div>
                        ...
                        <div className={`flex flex-col items-center justify-center space-y-3 sm:space-y-4 transition-all duration-1000 delay-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                            ...
                            <div className='flex flex-row space-x-2 text-base sm:text-lg'>
                        ...
                        <div className={`grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6 max-w-4xl mx-auto px-4 sm:px-0 transition-all duration-1000 delay-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                            {[
                                ...
                                <div key={index} className="text-center group hover:scale-105 transition-transform duration-300">
                                    <div className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white mb-1 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-300">{stat.number}</div>
